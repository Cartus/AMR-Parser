...
def score_for_all(generated_amr_file, gold_amr_file):
    """
    Main function of smatch score calculation for all instances (during dev/test)

    """
    generated_amr = open(generated_amr_file)
    gold_amr = open(gold_amr_file)
    ...
    while True:
        """
        args.f[0]: first AMR file
        args.f[1]: second AMR file
        """
        cur_amr1 = amr.AMR.get_amr_line(generated_amr)
        cur_amr2 = amr.AMR.get_amr_line(gold_amr)
        ...
        if not single_score:
            ...
            print(floatdisplay % best_f_score)
...
def get_specific_instance(gold_amr_file, instance_id):
    gold_amr = open(gold_amr_file)
    return amr.AMR.get_amr_specific_line(gold_amr, instance_id)


def get_all_instance(gold_amr_file):
    gold_amr = open(gold_amr_file)
    gold_amr_list = list()
    while True:
        amr_string = amr.AMR.get_amr_line(gold_amr)
        if amr_string == "":
            break
        gold_amr_list.append(amr_string)

    return gold_amr_list


def get_score_train(cur_amr1, cur_amr2):
    global verbose
    global veryVerbose
    global iteration_num
    global single_score
    ...
    if not single_score:
        ...
        if pr_flag:
            ...
            print("Recall: " + floatdisplay % recall)
        # print("F-score: " + floatdisplay % best_f_score)
        return (floatdisplay % best_f_score)
    total_match_num += best_match_num
    total_test_num += test_triple_num
    total_gold_num += gold_triple_num
    # clear the matching triple dictionary for the next AMR pair
    ...
    if single_score:
        ...
        #     print("Recall: " + floatdisplay % recall)
        # print("F-score: " + floatdisplay % best_f_score)
        return (floatdisplay % best_f_score, floatdisplay % precision, floatdisplay % recall)


def get_score_dev(cur_amr1_list, cur_amr2_list):
    global verbose
    global veryVerbose
    global iteration_num
    global single_score
    ...
    print("In the evaluation part: ")

    for idx in range(len(cur_amr1_list)):
        print(idx)
        cur_amr1 = cur_amr1_list[idx]
        cur_amr2 = cur_amr2_list[idx]
        # print(cur_amr2)
